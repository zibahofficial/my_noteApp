...
# Route alias: the deployed frontend calls the API without the "/api" prefix
# (e.g. /auth/login, /notes). Rewrite those root-level paths to the /api routes
# below so both conventions work. Must be registered BEFORE CORS so that
# CORSMiddleware stays the outermost middleware and answers OPTIONS preflights.
async def _root_api_alias(request: Request, call_next):
...
app.add_middleware(BaseHTTPMiddleware, dispatch=_root_api_alias)

# CORS - added last so it is the outermost middleware; reads CORS_ORIGINS from env.
app.add_middleware(
    CORSMiddleware,
